...
def risk_management_agent(state: AgentState, agent_id: str = "risk_management_agent"):
    """Controls position sizing based on volatility-adjusted risk factors
    and the user's risk appetite from their profile.

    User profile (PostgreSQL) sets:
      - conservative  → max 5% per position, reject >20% annual vol
      - moderate      → max 10% per position, reject >35% annual vol
      - aggressive    → max 20% per position, reject >50% annual vol
    """
    ...
    for ticker in tickers:
        ...
        # Volatility-adjusted limit pct (base)
        vol_adjusted_limit_pct = calculate_volatility_adjusted_limit(
            vol_data.get("annualized_volatility", 0.25)
        )

        # Apply user risk appetite multiplier from PostgreSQL profile
        if risk_appetite == "conservative":
            risk_multiplier = 0.5   # halve the position limit
        elif risk_appetite == "aggressive":
            risk_multiplier = 1.5   # 50% more aggressive sizing
        else:
            risk_multiplier = 1.0   # moderate = no adjustment
        vol_adjusted_limit_pct *= risk_multiplier
